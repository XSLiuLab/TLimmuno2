---
title: "Immunoeniting signal detect"
...
DF15 = pd.read_csv("/Users/wangguangshuai/Data/MHCII/Data/IMM_model_benchmark/DF15.csv")
```

```{python}
result_15 = pd.read_csv("/Users/wangguangshuai/Data/MHCII/Data/IMM_model_benchmark/15mer_rank_result.csv")
...
we download 33 cancer type somatic mutation file form [ucsx xena](https://xenabrowser.net/datapages/), we focus on "MuTect2 Variant Aggregation and Masking". here the detail of dataset:
...
![TCGA-prediction](./figure/TCGA-prediction.png)

![TCGA-prediction-withvalue](./figure/TCGA-prediction-with-Discretevalues.png)
...
if a missense mutation is a neoantigen? for a missense mutation, if a subsequence min HLA type's rank <0.02, is neo(1) ,else not neo(0)
...
all_mut_ccf <- readRDS("../data/mut_ccf_tpm_neo.rds")
...
saveRDS(neo_nes,file = "../data/neo_es.rds")
```

```{r simulation,eval = FALSE}
#R
neo_nes <- readRDS("../data/neo_es.rds")
all_mut_ccf <- readRDS("mut_ccf_tpm_neo.rds")
all_mut_ccf <- all_mut_ccf %>%
  rename(ccf=ccf_hat) %>%
  mutate(neo=ifelse(neo=="neo","yes","no"))
...
saveRDS(res,file = "sim_2000_not_filter_driver.rds")

neo_nes <- readRDS("neo_es.rds")
sim_all <- readRDS("sim_2000_not_filter_driver.rds")
sim_all <- bind_rows(sim_all)
sim_all$cancer <- get_cancer_type(sim_all$sample,parallel = TRUE,cores = 20)
saveRDS(sim_all,file = "sim_add_cancer.rds")
```
