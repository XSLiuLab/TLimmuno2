...
To compare with existing tools, we performed benchmark study by using independent datasets. We first used the IEDB-benchmark dataset (Methods) to compare the performance of TLimmuno2 and NetMHCIIpan BA/EL model. This dataset contains 105 immunogenicity peptides, 4 non-immunogenicity peptides and 5 times randomly selected human non-immunogenicity peptides. In this and all of the following validation processes, data that appeared in the training dataset were removed and thus were completely independent of the training set. TLimmuno2 shows much better performance (AUC: 0.9145) compared with NetMHCIIpan in this independent dataset (BA AUC = 0.7416, EL AUC = 0.7194).
...
```{python}
data = pd.read_csv("../data/IEDB_benchmark_with_random.csv")
data["len"] = data["Description"].map(len)
data = data[data["len"]>=15]
data["Description"].to_csv("../data/pep.csv",index = None,header = None)
res = pd.read_csv("../data/pep_result.csv")
res = pd.merge(res,data,left_on = "Peptide",right_on = "Description")
fpr,tpr,_ = roc_curve(res["label"],1-res["Immunogenicity Score"])
IEDB_AUC = auc(fpr,tpr)
```

```{python, Repitopes}
repitope_data = pd.read_csv("../data/IEDB_repitope.csv")
repitope_res = pd.merge(repitope_data,data,left_on = "Peptide",right_on = "Description")
rep_fpr,rep_tpr,_ = roc_curve(repitope_res["label"],repitope_res["ImmunogenicityScore"])
repitope_AUC = auc(rep_fpr,rep_tpr)
```
...
ax.plot(fpr,tpr,label='IEDB : (AUC={0:.4f})'.format(IEDB_AUC))
ax.plot(rep_fpr,rep_tpr,label='Repitope : (AUC={0:.4f})'.format(repitope_AUC))
