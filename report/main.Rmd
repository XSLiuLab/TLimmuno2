---
title: "TLimmuno2: predicting HLA class II antigen immunogenicity through transfer learning"
...
> Of note, some step was done on Linux server provided by High Performance Computing(HPC) Service of ShanghaiTech University in order to reduce run time. So some scripts was sunmitted by using PBS(Portable Bash Script) scripts, we will mark them in the header by using `#PBS`. Also, there are mixture of R and Python, if readers want to reproduce this work, please focus on the key code lines.
...
4.Background sequence: we download all human peptide sequence from UniPort database(2022.3) and randomly sampling peptides. We generated a set of 90,000 13--21-mer random natural peptides (10,000 of each length). The result maybe different as the reason of random select.
